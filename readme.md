...
```angular2html
    ...
    二、音视频同步问题：当成功切换流之后 还是使用旧的pts、dts会导致音视频不同步问题
        ...
        切流之后清理所有的音视频缓存包括解码队列和fifo缓存(已解决)

        接入新流之后判断实时pts、dts 选择性编码(实现难度大)
        
        新思路: 实际上清理之后 接入的就是同步的 但是存在一个问题 
            由于互斥量和条件变量(保证线程安全),很有可能提出切流指令
            但等待一些残留数据发送后 才切换流 导致音视频不同步问题
            暂时解决思路就是在这 要么彻底清除 切流指令一发送编码发送停止
            要么补齐 我缺多少 补完多少才切换完成
        

        视频还在切换 音频已经正常播放
        视频渲染时间远大于音频  为了保证同步得保留一部分音频？
