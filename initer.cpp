...
//    av_dict_set(&options,"list_devices", "true", 0);
    ...
    // 打开视频设备
    //video_input_fmt = av_find_input_format("video4linux2");
    ...
    // audio_input_fmt = av_find_input_format("alsa");
//    audio_input_fmt = av_find_input_format("dshow");
//   // if (avformat_open_input(&fmt_ctx, "default", audio_input_fmt, nullptr) != 0) {
//    if(avformat_open_input(&fmt_ctx, "audio=Headphone (Realtek(R) Audio)", audio_input_fmt, nullptr) != 0){
...
//        return -1;
//    }
    //AVStream* audioStream = device_ifmt_ctx->streams[audioStreamIndex];

    av_dump_format(fmt_ctx, 0, "video=HP Wide Vision HD Camera", 0);
   // av_dump_format(fmt_ctx, 0, "video=dummy", 0);
...
void IniterD::show_device_info(const std::string& device_name) {
    AVDictionary* options = nullptr;
    av_dict_set(&options,"list_devices", "true", 0);
