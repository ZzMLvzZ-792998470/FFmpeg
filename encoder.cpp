...
Encoder::Encoder(int height, int width, int framerate, int samplerate, int encoder_type) : height(height),
                                                                width(width),
                                                                framerate(framerate),
...
int Encoder::init_encoder(AVFormatContext *ofmt_ctx, int type) {
    if(encoder_type == 0) return init_encoder(ofmt_ctx);
    ...
    for (i = 0; i < 1; i++) {
        ...
        const AVCodec *encoder = encoder_type == 1 ? avcodec_find_encoder_by_name("libx264") : avcodec_find_encoder(AV_CODEC_ID_AAC);
...
//            enc_ctx->sample_rate = 44100;
    ...
    if(encoder_type == 1) ofmt_ctx->streams[0]->time_base = AVRational{1, 90000};
    else ofmt_ctx->streams[0]->time_base = AVRational{1, 44100};
    m_ofmt_ctx = ofmt_ctx;
    return 0;
...
int Encoder::encode_video_mp4(AVFrame *frame) {
    int ret;
    int stream_index = 0;
    AVFrame *encode_frame = frame;
    AVPacket *enc_pkt = av_packet_alloc();
    ...
    while (ret >= 0) {
        ...
        enc_pkt->pts = enc_pkt->dts = (enc_pkt->pts * m_ofmt_ctx->streams[stream_index]->time_base.den / m_ofmt_ctx->streams[stream_index]->time_base.num / 1000);
        Writer::write_packets(m_ofmt_ctx, enc_pkt);
...
int Encoder::flush() {
    ...
    encode_video_mp4(nullptr);
}


void Encoder::setAudioEncCtxParam() {
...
int Encoder::encode_video_rtmp(AVFrame *frame, int64_t &last_time) {
    //int64_t func_time = Timer::getCurrentTime();
    int ret;
    int stream_index = 0;
    AVFrame *encode_frame = frame;
...
int Encoder::encode_audio_rtmp(AVFrame *frame, int64_t &last_time) {
    int64_t func_time = Timer::getCurrentTime();
    int ret;
    int stream_index = 1;
    AVFrame *encode_frame = frame;
...
int Encoder::encode_video_rtsp(AVFrame *frame, int64_t &last_time) {
    //int64_t func_time = Timer::getCurrentTime();
    int ret;
    int stream_index = 0;
    AVFrame *encode_frame = frame;
...
int Encoder::encode_audio_rtsp(AVFrame *frame, int64_t &last_time) {
    int64_t func_time = Timer::getCurrentTime();
    int ret;
    int stream_index = 1;
    AVFrame *encode_frame = frame;
...
int Encoder::encode_video_rtp(AVFrame *frame, int64_t &last_time) {
    //int64_t func_time = Timer::getCurrentTime();
    int ret;
   // int stream_index = 0;
    AVFrame *encode_frame = frame;
...
//        av_log(nullptr, AV_LOG_INFO, "video packet.\n");
        ...
        enc_pkt->pts = enc_pkt->dts = (enc_pkt->pts * m_ofmt_ctx->streams[0]->time_base.den / m_ofmt_ctx->streams[0]->time_base.num / 1000);
        ...
        Writer::write_packets(m_ofmt_ctx, enc_pkt);
       // Writer::write_packets_no_interleaved(m_ofmt_ctx, enc_pkt);
...
int Encoder::encode_audio_rtp(AVFrame *frame, int64_t &last_time) {
    int64_t func_time = Timer::getCurrentTime();
    int ret;
    //int stream_index = 1;
    AVFrame *encode_frame = frame;
...
int Encoder::encode_video_test(AVFrame *frame, int64_t &last_time) {
    int ret;
    int stream_index = 0;
    AVFrame *encode_frame = frame;
    AVPacket *enc_pkt = av_packet_alloc();
...
int Encoder::encode_audio_test(AVFrame *frame, int64_t &last_time) {
    int64_t func_time = Timer::getCurrentTime();
    int ret;
    int stream_index = 1;
    AVFrame *encode_frame = frame;
