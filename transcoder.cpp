...
int Transcoder::device2mp4() {
    ...
    Thread thread_encode_video(&Transcoder::mp4_video_deviceSeparate, this, std::ref(time_s), std::ref(video_over));
    //thread_encode_video.detach();

    //Thread thread_encode_audio(&Transcoder::mp4_audio_deviceSeparate, this, std::ref(time_s), std::ref(audio_over));
    //thread_encode_audio.detach();

    Thread thread_encode_audio(&Transcoder::mp4_audio_silentFrameSeparate, this, std::ref(time_s));
...
//    thread_decode_audio.detach();
    ...
    Thread thread_video(&Transcoder::rtmp_video_deviceSeparate, this, std::ref(time), std::ref(video_over));

    Thread thread_audio(&Transcoder::rtmp_audio_silentFrameSeparate, this, std::ref(time));
    //Thread thread_audio(&Transcoder::rtmp_audio_deviceSeparate, this, std::ref(time), std::ref(audio_over));
...
//
//    Thread thread_encode_audio(&Transcoder::rtmp_audio_perSecondSeparate, this, std::ref(works));
...
int Transcoder::mp4_video_deviceSeparate(int& time, int& video_over) {
    int ret;
    ...
    while(video_frames_counts >= 0){
        ...
        if(!decoders[0]->get_video_queue().empty()){
            ...
            encoders[0]->encode_video_mp4(Utils::change_frame_size(width, height, av_frame_clone(decoders[0]->get_video_queue_front())));
...
int Transcoder::mp4_audio_deviceSeparate(int& time, int& audio_over) {
    int ret;
            ...
            if(!decoders[1]->get_audio_queue().empty()){
                Utils::write_to_fifo(fifo, swr_ctx,
                        //av_frame_clone(decoders[1]->get_audio_queue().front()),
                                     av_frame_clone(decoders[1]->get_audio_queue_front()),
                                     encoders[0]->get_audio_enc_ctx()->channels,
                                     encoders[0]->get_audio_enc_ctx()->sample_fmt,
                                     encoders[0]->get_audio_enc_ctx()->sample_rate);
...
int Transcoder::mp4_video_blackFrameSeparate(int &time) {
    int ret;
...
int Transcoder::mp4_audio_silentFrameSeparate(int &time) {
    int ret;
...
int Transcoder::rtmp_video_perSecondSeparate(std::vector<int>& works) {
    //初始化视频帧编码需要的内容 前一帧+合并帧
    unsigned int i;
    bool video_finish;
...
int Transcoder::rtmp_audio_perSecondSeparate(std::vector<int>& works) {
    int ret;
    unsigned int i;
    AVFrame* silence_frame = FrameCreater::create_audio_frame();
    int64_t time_one_second = Timer::getCurrentTime();
...
int Transcoder::rtmp_video_deviceSeparate(int &time, int& video_over) {
    int ret;
...
int Transcoder::rtmp_audio_deviceSeparate(int &time, int& audio_over) {
    int ret;
    unsigned int i;
...
int Transcoder::rtmp_video_blackFrameSeparate(int &time) {
    int ret;
    AVFrame *black_frame = FrameCreater::create_video_frame();
...
int Transcoder::rtmp_audio_silentFrameSeparate(int &time) {
    int ret;
    AVFrame *silent_frame = FrameCreater::create_audio_frame();
    int audio_frames_counts = (encoders[0]->get_audio_enc_ctx()->sample_rate / encoders[0]->get_audio_enc_ctx()->frame_size) * time;
    int per_second_counts = 0;
